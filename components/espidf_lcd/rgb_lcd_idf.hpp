...
// LCD device addresses
constexpr uint8_t LCD_ADDRESS      = 0x3E;
constexpr uint8_t RGB_ADDRESS      = 0x62; // classic
constexpr uint8_t RGB_ADDRESS_V5   = 0x6A; // newer v5

// LCD command flags (as in original)
...
public:
  ...
  esp_err_t init_i2c(i2c_port_t port, gpio_num_t sda, gpio_num_t scl, uint32_t clk_hz = 100000);
...
private:
  ...
  uint8_t rgb_addr_ = RGB_ADDRESS;
  bool initialized_ = false;

  // LCD state from original
