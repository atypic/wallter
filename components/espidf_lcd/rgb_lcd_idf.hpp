...
constexpr uint8_t RGB_ADDRESS_V5 =
    0x30; // newer chip variant (matches Grove rgb_lcd v5)

// Some clones use other addresses; keep as probe candidates in the
// implementation.
...
class rgb_lcd_idf {
public:
  esp_err_t init_i2c(i2c_port_t port, gpio_num_t sda, gpio_num_t scl,
                     uint32_t clk_hz);
  esp_err_t begin(uint8_t cols, uint8_t lines, uint8_t dotsize);

  // High-level text API
  esp_err_t soft_reinit();
  esp_err_t clear();
  esp_err_t home();
