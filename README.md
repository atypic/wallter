...
When Wallter powers on, the display shows the firmware version (e.g. `v2.2.0 ARCTIC_CYTRON`) for 2 seconds, then runs through a startup animation. What happens next depends on which buttons are held during power-on:
...
Use the **Up** / **Down** buttons to step through target angles, same as the physical buttons.
...
| **Max extend speed** | Maximum motor speed when extending. `0` = use firmware default. |
| **Max retract speed** | Maximum motor speed when retracting. `0` = use firmware default. |
