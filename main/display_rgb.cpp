...
Display::Display()
    : lcd(), current_view(LCD_HOMING_VIEW), next_view(LCD_HOMING_VIEW),
      previous_view(LCD_HOMING_VIEW), last_update_ms(0), last_reinit_ms(0),
      refresh_interval_ms(REFRESH_INTERVAL), short_view_time_ms(0),
      short_view_start_ms(0), pending_refresh(false) {}
...
void Display::init(void) {
  ...
  if (lcd.begin(16, 2, 0) != ESP_OK) {
    ...
    // Expected for Grove/JHD1313M1-style RGB LCD: LCD 0x3E, RGB 0x62 (or 0x6A
    // for v5).
    for (uint8_t addr :
         {uint8_t(0x3E), uint8_t(0x3F), uint8_t(0x62), uint8_t(0x6A)}) {
      esp_err_t perr = probe_i2c_address(I2C_NUM_0, addr);
      ESP_LOGI("Display", "I2C probe 0x%02X -> %s", addr,
  ...
  for (uint8_t addr :
       {uint8_t(0x30), uint8_t(0x3E), uint8_t(0x62), uint8_t(0x6A)}) {
    esp_err_t perr = probe_i2c_address(I2C_NUM_0, addr);
...
void Display::print(const char *message, const char *line2) {
  // Avoid lcd.clear() to prevent visible flicker; instead overwrite both
  // 16-char lines. Pad with spaces so shorter strings fully replace previous
  // content.
...
void Display::refresh() {
  ...
  // Periodic LCD register refresh to recover from noise-induced corruption.
  // Only re-sends config registers (no clear), so no visible flicker.
  ...
  if (!pending_refresh && (now - last_update_ms) < refresh_interval_ms)
    return;
...
void Display::update_target_view(float target_angle, uint8_t percent) {
  ...
  uint8_t filledCells = (clamped == 100)
                            ? innerWidth
                            : (uint8_t)((clamped * innerWidth + 99) / 100);
  // Build bar in-place
...
void Display::update_manual_view_raw(const char *line1, const char *line2) {
  snprintf(view_buffers[LCD_MANUAL_VIEW][0], 17, "%-16.16s",
           line1 ? line1 : "");
  snprintf(view_buffers[LCD_MANUAL_VIEW][1], 17, "%-16.16s",
           line2 ? line2 : "");
  next_view = LCD_MANUAL_VIEW;
}
